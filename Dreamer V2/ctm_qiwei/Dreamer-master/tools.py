  ...
  while True:
    ...
    for index in random.choice(len(keys), rescan):
      ...
      if length:
        ...
        if total < length:
          if total == 0:
            raise ValueError('Cannot sample an empty episode')
          # Keep short failures. Repeat the final observation for batching,
          # but exclude padding from every training objective using valid.
          episode = {k: np.concatenate(
              [v, np.repeat(v[-1:], length - total, axis=0)], axis=0)
              for k, v in episode.items()}
          episode['valid'] = np.concatenate([
              np.ones(total, np.float32), np.zeros(length - total, np.float32)])
          episode['action'][total:] = 0
          episode['reward'][total:] = 0
          episode['discount'][total:] = 0
          yield episode
          continue
        ...
        episode['valid'] = np.ones(length, np.float32)
      yield episode
...
def lambda_return_from_next_value(reward, next_value, discount, lambda_):
  """Time-major targets for a_t: r_(t+1), V(s_(t+1)), gamma_(t+1).

  Unlike lambda_return(), values here already refer to successor states.
  The last transition bootstraps from its own successor, without shifting.
  """
  inputs = reward + discount * next_value * (1 - lambda_)
  return static_scan(
      lambda carry, item: item[0] + item[1] * lambda_ * carry,
      (inputs, discount), next_value[-1], reverse=True)


def masked_mean(value, weight):
  value, weight = tf.cast(value, tf.float32), tf.cast(weight, tf.float32)
  return tf.math.divide_no_nan(
      tf.reduce_sum(tf.where(weight > 0, value * weight, 0.0)),
...
class Adam(tf.Module):
  ...
  def initialize(self):
    """Materialize optimizer slots without changing model parameters."""
    self._ensure_variables()
    build = getattr(self._opt, 'build', None)
    if build:
      build(self._variables)
