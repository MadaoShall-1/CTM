...
class NavigationFixes(unittest.TestCase):
  ...
  def test_action_masks_and_gradients(self):
    action = tf.Variable([[1.,0.,.2,.7], [0.,1.,.4,.5]])
    with tf.GradientTape() as tape:
      actual = models.canonical_action(action)
      loss = tf.reduce_sum(actual[...,2:])
    np.testing.assert_allclose(actual.numpy(), [[1,0,.2,0],[0,1,0,.5]])
    np.testing.assert_allclose(tape.gradient(loss, action).numpy()[...,2:], [[1,0],[0,1]])
    np.testing.assert_array_equal(models.canonical_action(tf.zeros([1,4])), np.zeros([1,4]))
    env = DreamerV2UAVEnv('relay')
  ...
  def test_imagination_pairs_actions_with_source_states(self):
    ...
    class Agent:
      ...
      _actor_dist = staticmethod(Distribution)
    actor_inputs, actor_features, features, actions = dreamer.DreamerV2._imagine(
        Agent(), {'deter': tf.constant([[[2.]]])})
    np.testing.assert_allclose(actor_inputs[:,0,0], [2,3,4])
    np.testing.assert_allclose(actor_features[:,0,0], [2,3,4])
    np.testing.assert_allclose(features[:,0,0], [3,4,5])
  ...
  def test_safe_reward_has_no_early_failure_bonus(self):
    ...
    obs, reward, done, info = env.step(np.asarray([1,0,0,0], np.float32))
    self.assertFalse(done); self.assertEqual(obs['vector'][10], 1.)
  ...
  def test_training_and_policy_without_images(self):
    ...
    for task in ('direct', 'relay'):
      with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        env = DreamerV2UAVEnv(task, max_episode_steps=20)
        obs = env.reset()
        ...
        agent = dreamer.DreamerV2(
            c, root/'episodes', env.action_space, env.observation_space, None)
