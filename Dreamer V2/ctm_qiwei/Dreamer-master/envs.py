...
class DreamerV2UAVEnv:
    """Expose UAV dynamics with an explicitly versioned task objective.

    goal_safe_v1 uses finite-horizon failure semantics and potential shaping;
    legacy retains the previous sampling/rewards for comparison. The action
    ...
    def __init__(
        ...
        self.seed = seed
        if reward_mode not in ("legacy", "goal_safe_v1"):
            raise ValueError(f"Unknown reward mode: {reward_mode}")
        if not 0.0 < reward_discount <= 1.0 or shaping_scale < 0:
            raise ValueError('Require 0 < reward_discount <= 1 and shaping_scale >= 0')
    ...
    def observation_space(self):
        ...
        for key in ("base_reward", "shaping_reward"):
            result[key] = spaces.Box(-np.inf, np.inf, shape=(), dtype=np.float32)
    ...
    def decode_action(self, action):
        ...
        effective = uav_actions.canonicalize_numpy(action, self.num_actions)
        parameter = np.asarray([effective[self.num_actions + discrete]], np.float32)
    ...
    def reset(self):
        ...
        if self.reward_mode == 'goal_safe_v1':
            for _ in range(10000):
                if self._valid_layout():
                    break
                obs, info = self.env.reset()
            else:
                raise RuntimeError('Could not sample a nondegenerate layout within the route budget')
        self.last_obs = obs
    ...
    def step(self, action):
        ...
        info = dict(info)
        if self.reward_mode == 'goal_safe_v1':
            # A boundary violation takes precedence over simultaneous delivery.
            success = bool(info.get('is_success', False)) and not info.get('out_of_bounds', False)
            info['is_success'] = success
            base_reward = (1.0 if success else -1.0) if done else -(1.0 - self.reward_discount)
            # Time is part of the state: hitting the horizon is a real task
            # failure, not an artificial truncation of a continuing task.
            next_potential = 0.0 if done else self._potential()
            shaping_reward = self.shaping_scale * (
                self.reward_discount * next_potential - previous_potential)
            reward = base_reward + shaping_reward
        else:
            base_reward, shaping_reward = reward, 0.0
        discount = 0.0 if terminated or (done and self.reward_mode == 'goal_safe_v1') else 1.0
        info.update(discount=np.asarray(discount, np.float32),
                    terminated=bool(terminated), truncated=bool(truncated),
                    base_reward=float(base_reward), shaping_reward=float(shaping_reward),
    ...
    def _valid_layout(self):
        points = [self.env.state.position]
        if self.task == 'relay':
            points.extend([self.env.relay_goal, self.env.final_goal])
        else:
    ...
    def _potential(self):
        radius = self.env.goal_radius
        if self.task == 'relay' and self.env.current_phase == 0:
            remaining = max(0.0, float(distance(self.env.state.position, self.env.relay_goal)) - self.env.relay_radius)
            remaining += max(0.0, float(distance(self.env.relay_goal, self.env.final_goal)) - radius)
        else:
            remaining = max(0.0, float(distance(self.env.state.position, self.env.current_goal)) - radius)
        scale = math.sqrt(2.0) * self.env.map_size * self.env.num_phases
    ...
    def _convert_obs(self, obs, info=None):
        ...
        out = {
            ...
            "vector": self._vector_observation(obs),
        }
        for key in ("is_success", "out_of_bounds", "terminated", "truncated",
                    "relay_reached", "carrying_supply"):
            out[key] = np.asarray(float(bool(info.get(key, False))), np.float32)
        for key in ('base_reward', 'shaping_reward'):
            out[key] = np.asarray(info.get(key, 0.0), np.float32)
        return out
